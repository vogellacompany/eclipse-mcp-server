...
A widget print goes through `gtk_widget_draw`, which paints in GTK's logical coordinates, so a 1:1 canvas of device size leaves the picture at 1x in the corner; the print goes into an image built from `ImageData`, whose surface carries the device scale, and GTK rasterises glyphs at that resolution.
It must not be a 1:1 canvas plus a scale `Transform`: `GC.fillGradientRectangle` sets the target surface's device scale and never restores it, so every stack the Light theme draws a gradient in comes out doubled, see `docs/platform-bugs.md`.
Measured against the X server rather than reasoned about: on a 3200x2000 Xvfb screen with `GDK_SCALE=2`, the root capture is now pixel identical to `import -window root` (RMSE 0, where it was a 400x300 downsample).
...
**`eclipse_hot_code_replace` keeps its agent in the system class loader, and attaches through a helper process.**
A JVM refuses `VirtualMachine.attach` on its own pid unless started with `jdk.attach.allowAttachSelf`, which is read from the saved startup properties, so setting it at runtime does nothing; `HotSwapAttacher` runs in a JVM of its own and reaches the attach API reflectively, so the bundle never imports `com.sun.tools.attach`.
`HotSwapAgent` is packed into a jar from this bundle's own class files and loaded from there, which puts the copy holding the `Instrumentation` into the system class loader, where it outlives any update of this bundle; `HotSwapSupport.existing` therefore reads it from `ClassLoader.getSystemClassLoader()` by name, never through the bundle's own copy of the class, which has no instrumentation and never will.
